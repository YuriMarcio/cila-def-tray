...
const FailedConfigurationError_1 = require("./errors/FailedConfigurationError");
const FailedRemoveDynamicFile_1 = require("./errors/FailedRemoveDynamicFile");
const FailedRemoveStaticFile_1 = require("./errors/FailedRemoveStaticFile");
const InvalidLayoutError_1 = require("./errors/InvalidLayoutError");
const InvalidOrNotSentParamsError_1 = require("./errors/InvalidOrNotSentParamsError");
const ResourceNotFoundError_1 = require("./errors/ResourceNotFoundError");
const UnknownError_1 = require("./errors/UnknownError");
const IsFileAllowed_1 = require("./utils/IsFileAllowed");
const KeysToCamel_1 = require("./utils/KeysToCamel");
const { appendFile } = fs_1.promises;
...
class Api {
     ...
     */
    constructor({ key, password, themeId, debug = false }) {
        this.debugFilePath = './.debug.sdk.log';
        this.version = '1.0.4';
        this.url = 'https://opencode.tray.com.br/api';
        this.key = key;
    ...
    verifyAuthenticationError(error) {
        if (error.response) {
            ...
            if (data.message === 'Token de acesso inválido' && data.code === '00001' && data.status === 401) {
                return new AuthenticationError_1.AuthenticationError(data.message, data);
            }
        }
        return false;
    }
     ...
     * @private
     */
    generateDebugFile({ type, operation, data }) {
        if (this.debug) {
            const date = new Date().toLocaleString('pt-br');
            const convertedData = typeof data === 'object' && data !== null ? JSON.stringify(data) : data;
            const dataToWrite = `[${date}] Type: ${type} | Operation: ${operation} | Data: ${convertedData}${os_1.EOL}`;
    ...
    checkConfiguration() {
        ...
        return axios_1.default
            ...
            const data = { authentication, themeId, preview };
            ...
            if (!sdkError) {
                if (error.response && 'authentication' in error.response.data) {
                    sdkError = new FailedConfigurationError_1.FailedConfigurationError(error.response.data);
                }
            }
            this.generateDebugFile({ type: 'Error', operation: 'checkConfiguration', data: error });
            return Promise.reject(sdkError || new UnknownError_1.UnknownError());
        });
    }
    /**
    ...
    cleanCache(themeId = this.themeId) {
        ...
        return axios_1.default
            ...
            if (response.data.response.code !== 200) {
                throw new UnknownError_1.UnknownError(`Unknown error. Details: ${response.data.response.message}`);
            }
    ...
    createTheme(name, base = 'default') {
        ...
        return axios_1.default
            ...
            const data = { themeId, name, preview, published };
            ...
            if (!sdkError && error.response && error.response.data.code === '00101') {
                sdkError = new InvalidOrNotSentParamsError_1.InvalidOrNotSentParamsError(error.response.data);
            }
        ...
        return axios_1.default
            ...
            if (!sdkError && error.response && error.response.data.message) {
                ...
                if (error.response.data.code === '00301') {
                    sdkError = new InvalidLayoutError_1.InvalidLayoutError(error.response.data);
                }
            }
    ...
    getAssets() {
        ...
        return axios_1.default
            .request(config)
            .then((response) => {
            const assets = (0, KeysToCamel_1.keysToCamel)(response.data.assets);
            const quantity = response.data.meta.total;
            const data = { assets, quantity };
    ...
    getAsset(asset) {
        ...
        return axios_1.default
            .request(config)
            .then((response) => {
            const responseData = (0, KeysToCamel_1.keysToCamel)(response.data);
            const assetContentBuffer = Buffer.from(responseData.content, 'base64');
            return (0, file_type_1.fromBuffer)(assetContentBuffer).then((fileType) => {
     ...
     */
    sendAsset({ asset, data, isBinary = false }) {
        const config = {
            url: `${this.url}/themes/${this.themeId}/assets`,
            method: 'put',
            headers: this.headers,
        ...
        return (0, IsFileAllowed_1.isFileAllowed)(asset).then(() => {
            return axios_1.default
                .request(config)
                .then((response) => {
        ...
        return axios_1.default
            ...
            if (!sdkError && error.response && error.response.data.message) {
                ...
                if (error.response.data.code === '00101') {
                    ...
                    sdkError = new ResourceNotFoundError_1.ResourceNotFoundError(error.response.data);
                }
                else if (error.response.data.code === '00204') {
                    sdkError = new FailedRemoveDynamicFile_1.FailedRemoveDynamicFile(error.response.data);
                }
                else if (error.response.data.code === '00205') {
                    sdkError = new FailedRemoveStaticFile_1.FailedRemoveStaticFile(error.response.data);
                }
            }
